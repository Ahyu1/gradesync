...
document.addEventListener('DOMContentLoaded', function() {
    const recordsContainer = document.querySelector('.disapprovedContainer');
    ...
    async function fetchRecords() {
        ...
        try {
            const response = await fetch(`/getDisapprovedRecordsByUser?username=${username}`);

            if (response.ok) {
                const records = await response.json();
                renderRecords(records); 
            } else {
                window.alert("You don't have any disapproved class record.");
            }
        } catch (err) {
