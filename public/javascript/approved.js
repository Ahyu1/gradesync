...
document.addEventListener('DOMContentLoaded', function() {
    const recordsContainer = document.querySelector('.approvedContainer');
    ...
    async function fetchRecords() {
        ...
        try {
            const response = await fetch(`/getApprovedRecordsByUser?username=${username}`);

            if (response.ok) {
                const records = await response.json();
                renderRecords(records); 
            } else {
                window.alert("You don't have any approved class record.");
            }
        } catch (err) {
